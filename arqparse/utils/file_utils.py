...
from arqparse.utils.logger import log

# SNI_DOMAINS загружаются динамически чтобы избежать циклического импорта
def _get_sni_domains():
    """Загружает SNI_DOMAINS из arqparse.config.settings (или пустой список)."""
    try:
        import arqparse.config.settings as settings
        return getattr(settings, 'SNI_DOMAINS', [])
    except Exception:
        return []
...
def split_file_by_size(filepath: str, max_size_mb: float = 49.0) -> List[str]:
    ...
    if file_size_bytes <= max_size_bytes:
        log("File size OK, no splitting needed")
        return [filepath]
...
def extract_ip_from_config(config_line: str):
    ...
    # Extract host from arqparse.config line
...
def _has_insecure_query_params(config_lower: str) -> bool:
    """Check for insecure parameters in the query string (VLESS, Trojan, etc.)."""
    # Check for allowInsecure in query parameters
    if 'allowinsecure=' in config_lower:
        match = _ALLOWINSECURE_PATTERN.search(config_lower)
        if match and match.group(1).strip() in ['1', 'true', 'yes', 'on']:
            return True

    # Check for insecure in query parameters
    if 'insecure=' in config_lower:
        match = _INSECURE_PATTERN.search(config_lower)
        if match and match.group(1).strip() in ['1', 'true', 'yes', 'on']:
            return True

    # Check for skip-cert-verify
    if 'skip-cert-verify=' in config_lower:
        match = _SKIPCERT_PATTERN.search(config_lower)
        if match and match.group(1).strip() in ['1', 'true', 'yes', 'on', 'enabled']:
            return True

    # Check for security=none or verify=0
    if 'security=none' in config_lower or 'verify=0' in config_lower or 'verify=false' in config_lower:
        return True
...
def _is_vmess_insecure(config_line: str) -> bool:
    """Check for insecure settings in VMess JSON configuration."""
    try:
        ...
        if decoded.startswith('{'):
            ...
            if j.get('insecure') in [True, 'true', 1, '1'] or j.get('allowInsecure') in [True, 'true', 1, '1']:
                return True
            # Check for security=none
            if str(j.get('scy', j.get('security', ''))).lower() == 'none':
                return True
            # Check for legacy VMess mode (alterId > 0)
            aid = j.get('aid', j.get('alterId'))
...
def _is_ss_insecure(config_line: str) -> bool:
    """Check for weak Shadowsocks encryption methods."""
    try:
        ss_part = config_line[5:]
        # Format: ss://method:password@host:port OR ss://base64(method:password)@host:port
        if ':' in ss_part and '@' in ss_part and ss_part.index(':') < ss_part.index('@'):
            method = ss_part.split(':')[0].lower()
            return method in _WEAK_SS_METHODS
        elif '@' in ss_part:
...
def _is_ssr_insecure(config_line: str) -> bool:
    """Check for weak ShadowsocksR encryption methods."""
    try:
...
def has_insecure_setting(config_line: str) -> bool:
    """Check if a config has insecure settings (Entry point)."""
    if not config_line:
        return False
    ...
    if config_line.startswith("vmess://"):
        if _is_vmess_insecure(config_line):
            return True
    elif config_line.startswith("ss://"):
        if _is_ss_insecure(config_line):
            return True
    elif config_line.startswith("ssr://"):
        if _is_ssr_insecure(config_line):
            return True

    # General query parameter checks (VLESS, Trojan, etc.)
    return _has_insecure_query_params(config_lower)
...
def apply_sni_cidr_filter(configs: List[str], filter_secure: bool = True) -> List[str]:
    ...
    from arqparse.core.settings_xray import SNI_DOMAINS
