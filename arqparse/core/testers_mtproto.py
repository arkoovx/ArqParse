...
from arqparse.core.parser import parse_mtproto_url
...
def _is_valid_mtproto_response(data: bytes) -> Tuple[bool, str]:
    ...
    # MTProto obfuscated2 ответ обычно начинается не с HTTP и не с нулей
    for prefix in _HTTP_PREFIXES:
        if data.upper().startswith(prefix):
            return False, "http_like"
            
    if data.startswith(b"\x00\x00\x00\x00") or data == b"\x00" * len(data):
        return False, "all_zeros"
        
    # Проверка энтропии (MTProto трафик зашифрован и выглядит как случайные данные)
    sample = data[:64]
    unique = len(set(sample))
    if unique < 12: # Увеличиваем порог уникальных байт
