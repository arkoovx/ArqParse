...
try:
    from arqparse.config.constants import XRAY_BASE_PORT, XRAY_PORT_RANGE, MAX_SAFE_CONCURRENCY
except ImportError:
...
def _parse_hysteria2_to_outbound(url: str, tag: str = "proxy") -> Optional[Dict]:
    ...
    try:
        # Убираем протокол для корректного парсинга urlparse
        temp_url = url.replace('hysteria2://', '//').replace('hy2://', '//')
        ...
        # Если в URL есть password (через @), используем его
        if not password and ':' in parsed.netloc and '@' in parsed.netloc:
            # Случай ss-style или когда пароль в userInfo
            user_info = parsed.netloc.split('@')[0]
            password = unquote(user_info)
...
def _parse_tuic_url(url: str, tag: str = "proxy") -> Optional[Dict]:
    """Parse TUIC URL to Xray outbound."""
    try:
        temp_url = url.replace('tuic://', '//')
        parsed = urlparse(temp_url)
        params = parse_qs(parsed.query)
        ...
        # Формат: tuic://uuid:password@host:port?sni=...&alpn=...
        user_info = unquote(parsed.username) if parsed.username else ""
        password = unquote(parsed.password) if parsed.password else ""
        
        # В TUIC часто uuid в username, password в password
        uuid = user_info
...
@contextmanager
def temporary_socks_proxy(url: str, xray_path: str, startup_timeout: float = 2.0):
    """Поднимает временный локальный SOCKS-прокси через один рабочий Xray-конфиг."""
    with _xray_semaphore:
        if not xray_path or not os.path.exists(xray_path):
            raise RuntimeError(f"Xray не найден: {xray_path}")

        port = _get_next_port()
        config = _create_xray_config(url, port)
        if not config:
            raise RuntimeError("Не удалось создать Xray-конфиг для временного прокси")
        ...
        try:
            ...
            if not _wait_for_port(port, timeout=startup_timeout):
                raise RuntimeError("Временный SOCKS-прокси не запустился")
...
def _pre_validate_url(url: str) -> Tuple[bool, str]:
    ...
    if protocol == 'vless':
        ...
        try:
            url_part = url.replace('hysteria2://', '', 1).replace('hy2://', '', 1).split('#')[0].split('?')[0]
            if '@' not in url_part:
                # В Hysteria2 пароль может быть необязательным в некоторых случаях, 
                # но обычно он есть. Однако без @ это точно невалидный URL с паролем.
                pass
        except Exception:
            pass
    elif protocol == 'tuic':
        try:
            url_part = url.replace('tuic://', '', 1).split('#')[0].split('?')[0]
            if '@' not in url_part:
                return False, "TUIC missing UUID/password"
        except Exception:
            pass
...
def _create_multi_config(urls: List[str]) -> Tuple[Optional[Dict], Dict[int, str], List[Tuple[str, str]]]:
    ...
    for idx, url in enumerate(urls):
        ...
        # Use a temporary tag for initial parsing
        outbound = _url_to_outbound(url, "temp_proxy_tag")
        if not outbound:
            skipped_urls.append((url, "Failed to parse outbound"))
            _track_error("Failed to parse outbound", url)
            continue
        ...
        try:
            ...
            _track_error("Validation error", url)
            continue

        # Now that we know the config is valid, allocate a port
        port = _get_next_port()
        while port in used_ports:
            port = _get_next_port()
        
        # Update the outbound tag with the real port
        outbound["tag"] = f"proxy{port}"
...
def _test_multi_config_batch(
    ...
    completed_counter: List[int] = None,
    total_count: int = 0,
    ...
    with _xray_semaphore:
        ...
        # Add skipped URLs to results immediately
        for url, reason in skipped:
            _track_error(reason, url)
            results.append((url, False, 0.0))
            if completed_counter is not None and total_count > 0 and progress_func:
                with results_lock:
                    completed_counter[0] += 1
                    cur_c = completed_counter[0]
                    cur_s = suitable_counter[0] if suitable_counter else 0
                progress_func(cur_c, total_count, cur_s, required_count or 0)

        if not config or len(port_map) == 0:
            return results
        ...
        try:
            ...
            if len(all_ports) > 5:
                key_ports = [all_ports[0], all_ports[-1]]
                for p in key_ports:
                    if not _wait_for_port(p, timeout=2.5):
                        ready = False
                        break
                
                if not ready:
                    with ThreadPoolExecutor(max_workers=min(len(all_ports), 20)) as executor:
                        port_results = list(executor.map(lambda p: _wait_for_port(p, timeout=2.0), all_ports))
                        ready = all(port_results)
            else:
                with ThreadPoolExecutor(max_workers=min(len(all_ports), 20)) as executor:
                    port_results = list(executor.map(lambda p: _wait_for_port(p, timeout=2.0), all_ports))
                    ready = all(port_results)
            ...
            if not ready:
                # Xray failed to start or ports didn't open - mark all as failed
                for p in all_ports:
                    results.append((port_map[p], False, 0.0))
                    if completed_counter is not None and total_count > 0 and progress_func:
                        with results_lock:
                            completed_counter[0] += 1
                            cur_c = completed_counter[0]
